...
export const authAPI = {
  ...
  async signUp(email, password, name) {
    console.log(
      "🔐 AuthAPI: Attempting sign up for:",
      email,
      "with name:",
      name,
    );
    ...
    try {
      ...
      if (result.user || result.data) {
        ...
        const errorMsg =
          result.error?.message || result.error || "Sign up failed";
  ...
  async getSession() {
    ...
    if (!activeAuthClient) {
      console.error(
        "🔐 AuthAPI: No active auth client available for getSession",
      );
      return { success: false, error: "Auth client not available" };
    }
    ...
    try {
      ...
      return {
        success: false,
        error: error.message || "Session retrieval error",
      };
    }
  },
