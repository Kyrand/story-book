...
function processElement(element, polygon) {
  ...
  try {
    ...
    console.warn(
      "Error calculating distance for element:",
      element.id,
      distanceError,
    );
...
function determineFeatureType(tags) {
  ...
  if (tags.building) {
    return {
      type: "building",
      subtype:
        tags.building === "yes" ? "Building" : formatFeatureType(tags.building),
    };
  }
...
function getElementGeometry(element) {
  ...
  if (element.type === "relation" && element.members) {
    ...
    const wayMember = element.members.find(
      (member) => member.type === "way" && member.geometry,
    );
    if (wayMember && wayMember.geometry) {
      const coordinates = wayMember.geometry.map((node) => [
        node.lon,
        node.lat,
      ]);
      if (coordinates.length >= 2) {
