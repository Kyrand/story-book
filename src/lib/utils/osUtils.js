 ...
 */
export async function queryNearbyOSFeatures(coordinates, radiusMeters = 1000, apiKey) {
	...
	try {
		...
		results.forEach((result, index) => {
			...
			if (result.status === 'fulfilled') {
				...
				console.log(`✅ OS ${featureTypes[index]}: ${result.value.length} features`);
			} else {
				console.warn(`❌ OS ${featureTypes[index]} query failed:`, result.reason);
			}
		});
		...
		const sortedFeatures = processedFeatures.sort((a, b) => a.distance - b.distance);
...
async function queryOSFeatureType(featureType, polygonCoords, apiKey, polygon) {
	...
	try {
		console.log(`🔗 NGD Features API URL: ${ngdUrl.replace(apiKey, 'API_KEY_HIDDEN')}`);
		...
		if (!response.ok) {
			...
			console.error(`❌ OS Features API error: ${response.status} ${response.statusText}`);
			console.error(`Response body: ${errorText}`);
			throw new Error(`OS Features API error: ${response.status} ${response.statusText}`);
		}
		...
		if (data.features && data.features.length > 0 && featureType === 'Buildings') {
			console.log(
		...
		if (features.length > 0 && features[0].geometry) {
			console.log(`📍 Sample ${featureType} coordinates:`, features[0].geometry.coordinates.slice(0, 2));
		}
...
function buildOSWFSUrl(typeName, polygonCoords, apiKey) {
	...
	console.log(`🔧 Final NGD URL: ${finalUrl.replace(apiKey, 'API_KEY_HIDDEN')}`);
	return finalUrl;
}
...
function processOSFeature(feature, polygon) {
	...
	const { type, subtype } = determineOSFeatureType(feature.properties, feature._queryType);
	...
	try {
		...
		console.warn('Error calculating distance for OS feature:', feature.id, distanceError);
		distance = 9999;
	}
...
function determineOSFeatureType(properties, queryType) {
	...
	const descriptiveGroup = properties.descriptiveGroup || properties.theme || properties.function;
	const descriptiveTerm = properties.descriptiveTerm || properties.form || properties.class;
	...
	if (primaryType === 'highway' && (properties.roadClassification || properties.classification)) {
		subtype = formatFeatureType(properties.roadClassification || properties.classification);
	}

	// For buildings, use more specific info if available
	if (primaryType === 'building' && (properties.buildingFunction || properties.theme)) {
		subtype = formatFeatureType(properties.buildingFunction || properties.theme);
	}
