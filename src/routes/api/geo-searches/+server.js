...
export async function POST({ request, locals }) {
	try {
		...
		const { projectId, description, polygonData, features, proximityLimit } = await request.json();
		...
		// Start transaction
		const transaction = db.transaction((searchId, projectId, description, polygonData, proximityLimit, features) => {
			// Insert geo search
		...
		transaction(searchId, projectId, description, polygonData, proximityLimit, features);

		// Get the created search
		const newSearch = db.prepare(`
		...
		console.log(`✅ Geo search saved: ${description} (${features?.length || 0} features)`);
